...
```{r,include=FALSE,getdata}
#loading various packages that might come handy
library(ggplot2)
library(caret)
library(Hmisc)
library(rpart)
...
#current directory in "CourseProject/Writeup", data one up
relativedirectory <-"../"
datafile <- paste(relativedirectory, "/pml-training.csv", sep="")
...
*** =left
```{r, echo=FALSE, fig.width=6, fig.height = 6,featureplot}
featurePlot(x=training[,c("roll_belt","total_accel_arm", "pitch_dumbbell", "pitch_forearm")],y=training$classe,plot="pairs")
```
...
*** =left
```{r, echo=FALSE, fig.width=7, fig.height = 6,treeplot}
cvCtrl <- trainControl(method = "cv", number = 5,  selectionFunction <- "best")
fitMod <- train(classe ~ ., data = training, method = "rpart",trControl = cvCtrl)
...
```{r, traincm, echo = FALSE}
predictontraining <- predict(fitMod$finalModel, newdata = training, type = "class")
cm <- confusionMatrix(predictontraining, training$classe)

round(cm$table / colSums(cm$table), 3)
```
...
```{r, testcm, echo = FALSE}
predictontesting <- predict(fitMod$finalModel, newdata = testing, type = "class")
cm <- confusionMatrix(predictontesting, testing$classe)
...
*** =left
```{r, pft, echo = FALSE, fig.width=7, fig.height = 6,}
testing$predRight <- predictontesting == testing$classe
qplot(roll_arm, yaw_dumbbell, col = predRight, data = testing)
```
